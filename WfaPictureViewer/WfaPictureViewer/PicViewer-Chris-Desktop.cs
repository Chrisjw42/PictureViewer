...
namespace WfaPictureViewer
{
    ...
    public partial class PicViewer : Form
    {
        ...
        float picBoxRatio;
        Color defaultBG;
        public List<LoadedImage> listLoadedImg;
        ...
        private Bitmap GetArgbVer(Image sourceImg)
        {
        ...
        private Bitmap ApplyTransparency(Bitmap passedImg, byte newAlphaAmount = 100)
        {
        ...
        private Bitmap ApplyGrayscale(Bitmap passedImg, string algorithm)
        {
            // Creating a new memory assignment, so the pointer(I think) doesn't chance the originalImg
            Bitmap sourceImg = new Bitmap(passedImg);
        ...
        private Bitmap ApplySepia(Bitmap passedImg)
        {
            // Creating a new memory assignment, so the pointer(I think) doesn't chance the originalImg
            Bitmap sourceImg = new Bitmap(passedImg);
            ...
            /* using (OpenFileDialog dlgOpen = new OpenFileDialog())
             {
                 ...
                 {
                     // Every file that is arrIsProcessed in the dlg
